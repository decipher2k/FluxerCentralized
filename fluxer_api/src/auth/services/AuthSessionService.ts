...
import {syncSessionToSso, invalidateSessionInSso, invalidateAllUserSessionsInSso} from '~/sso/SsoSessionSync';
...
export class AuthSessionService {
	...
	async createAuthSession({user, request}: CreateAuthSessionParams): Promise<[token: string, AuthSession]> {
		...
		// Sync to global SSO session store (cross-instance awareness)
		void syncSessionToSso({authSession, request});
	...
	async revokeToken(token: string): Promise<void> {
		...
		// Invalidate in global SSO store (cross-instance propagation)
		void invalidateSessionInSso({sessionIdHash: tokenHash});
	...
	async logoutAuthSessions({user, sessionIdHashes}: LogoutAuthSessionsParams): Promise<void> {
		...
		// Invalidate each session in global SSO store
		for (const hash of hashes) {
			void invalidateSessionInSso({sessionIdHash: hash});
		}
	...
	async terminateAllUserSessions(userId: UserID): Promise<void> {
		...
		// Invalidate all user sessions in global SSO store
		void invalidateAllUserSessionsInSso(userId.toString());
